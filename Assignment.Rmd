...
In the observed regions 0 to 27 events were registered and the median number of observations is 5. The expected value of the cases was between 0.710 and 11.700 with median 6.365.

The two distribution of the data are the following:
...
First we visualize the distribution of the data on the map of the regions: 
```{r warning=FALSE}
...
<!-- We also want to view the $I_i$ statistic of the local Moran tests. So we create the intervals for $I_i$: [lowest $I_i$ value, -1], [-1,-0.5], [-0.5,0.5], [0.5, 1.5], [1.5, highest $I_i$ value]. The first two intervals are low $I_i$ values, the third interval corresponds to values close to 0, and the last two intervals are high $I_i$ values. -->
...
<!-- We can confirm the presence of a cluster of very high values in the center and we notice a cluster of quite high values in the north, which might indicate  -->
