...
To study the autocorrelation of the chain we look at the autocorrelation plots which WinBugs gives us.
...
The values of RR estimated by the model follow a radial pattern - with higher values in the center and gradually lower values far from the center. Its pattern is smoother than the one followed by the observed values of SMR, which present many low-value hotspots in the center and some high-value clusters in the north. 

We could say that the SMR has two main clusters (center and north), of which only one is recognized by the model (center), but from a qualitative point of view the chosen model can describe well the phenomenon, even though just in a rough way and not so in detail. 
...
Let's analyse the meaning of the SMR: a ratio greater than 1 indicates that more mortality has occurred with respect to the expected values, which means that we can state with pretty high probability that the black and brown regions have an increase in the value of the observed cases ($p>46\%$ for the brown regions and $p>80\%$ for the black ones).

We may suppose the presence of an underlying phenomenon which is causing this increase of the mortality due to larynx cancer. It may be linked to some change of the environmental-working conditions in the areas under investigation.
...
Notice that the formula is $RR[i] = e^{\alpha_0 + s[i]} = e^{\alpha_0} \cdot e^{s[i]}$ and the multiplying factor is exactly $e^{s[i]}$.
...
- In the regions where $s[i]>0$, so $e^{s[i]}>1$, RR is positively influenced by the spatial effect, leading to a higher number of deaths than expected
